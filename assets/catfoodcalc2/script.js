...
function goToStep(step) {
    ...
    // If going to step 4 (results), scroll to top of results section
    if (step === 4) {
        scrollToResultsTop();
    }
}
...
function displayResults(results) {
    ...
    // Display cat details with the cat's name as a header
    catDetails.innerHTML = `
        ...
        <div class="result-item calories-needs">
            <p class="result-title">Daily Caloric Needs:</p>
            <p class="calories-value">${results.dailyCalories.toFixed(1)} calories</p>
        </div>
        
        <div class="scientific-basis">
            <p>This advice is based on scientific research and veterinary nutritional guidelines.</p>
        </div>
    `;
    
    resultsContent.innerHTML = `
        <div class="food-recommendation">
            <h3>Dry Food Recommendation: ${results.dryFood.grams} grams per day</h3>
            <div class="recommendation-details">
                <p><strong>Food(s):</strong> ${results.dryFood.names.join(', ')}</p>
                <p><strong>Portion of daily intake:</strong> ${results.dryPercent ? Math.round(results.dryPercent) : 50}%</p>
                <p><strong>Energy density:</strong> ${results.dryFood.caloriesPerGram.toFixed(3)} kcal/g</p>
            </div>
        </div>
        
        <div class="food-recommendation">
            <h3>Wet Food Recommendation: ${results.wetFood.grams} grams per day</h3>
            <div class="recommendation-details">
                <p><strong>Food(s):</strong> ${results.wetFood.names.join(', ')}</p>
                <p><strong>Portion of daily intake:</strong> ${results.wetPercent ? Math.round(results.wetPercent) : 50}%</p>
                <p><strong>Energy density:</strong> ${results.wetFood.caloriesPerGram.toFixed(3)} kcal/g</p>
            </div>
        </div>
        
        <div class="result-item feeding-advice">
            <p class="result-title">Feeding Schedule and Tips:</p>
            <ul class="feeding-tips">
                <li><strong>Feed your cat 2-3 times per day</strong> at regular times.</li>
                <li><strong>Never "free feed" your cat.</strong> It can lead to chronic dehydration, obesity, diabetes, and stress.</li>
                <li><strong>Weigh your cat once per week</strong> to keep track of their condition.</li>
                <li>Make adjustments to portion sizes based on your cat's weight trends.</li>
            </ul>
        </div>
...
// Function to ensure the top of results section is visible
